...
namespace FluentSecurity.Policy
{
	public class RequireAllRolesPolicy : ISecurityPolicy
	{
		...
		public PolicyResult Enforce(ISecurityContext context)
		{
			...
			if (context.CurrentUserRoles() == null || context.CurrentUserRoles().Any() == false)
				return PolicyResult.CreateFailureResult(this, "Access denied");

			if (_requiredRoles.Any(requiredRole => context.CurrentUserRoles().Contains(requiredRole) == false))
			{
